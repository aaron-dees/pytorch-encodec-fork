    ...
    for i in range(start_epoch, config.diffusion.epochs):
        ...
        with torch.no_grad():
            ...
            if i % config.diffusion.view_analysis_interval == 0:
                ...
                if config.diffusion.scale_required == False:
                    scale_tensor = None
                rnd_encodec_frames = reconstruct_encodec_frames(diff, scale_tensor)
                pred_encodec_frames = reconstruct_encodec_frames(val_pred, scale_tensor)
                ...
                rnd_audio = w_model.decode(rnd_encodec_frames)
                ...
                writer.add_audio("Recon/RandLatent", rnd_audio[0], sample_rate=w_model.sample_rate, global_step=i+1)
